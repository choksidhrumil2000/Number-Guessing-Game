...
(async function main(start_game_again: boolean): Promise<void> {
    ...
    let answer: string = await new Promise<string>((resolve, reject) => {
        try {
            rl.question("Enter ['Y'==>'Yes'/'N'==>'No']: ", (inp: string) => {
                resolve(inp);
            });
        } catch (e) {
            reject("ERROR: Something Went Wrong!! FROM PHASE1 INPUT");
        }
    });
    ...
    if (isValid) {
        ...
        if (choice_of_difficulty === '4') {
            ...
            await gamePlay(MODES[parseInt(choice_of_difficulty) - 1].MODE, MODES[parseInt(choice_of_difficulty) - 1].CHANCES);
        }
    } else {
...
*********************************************************************`;
    ...
    const choice: string = await new Promise<string>((resolve,reject) => {
        try{
            rl.question("Enter Your Choice: ", (answer) => {
                resolve(answer);
            });
        }catch(e){
            reject("ERROR: Something Went Wrong!! FROM GUESS INPUT");
        }
...
async function gamePlay(mode: string, chances: number): Promise<void> {
    ...
    let score = chances;
    while (chances > 0) {
        let user_guess: string = await new Promise<string>((resolve,reject) => {
            try{
